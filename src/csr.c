...
int csr_read(cpu_t* cpu, uint32_t addr, uint64_t* out) {
    ...
    switch (addr) {
        ...
        case CSR_SATP:          *out = cpu->csr.satp; break;
        ...
        default:
            if (addr >= CSR_HPMCOUNTER3 && addr <= CSR_HPMCOUNTER3 + 0x1C) {
                *out = cpu->csr.hpmcounter3[addr - CSR_HPMCOUNTER3];
...
int csr_write(cpu_t* cpu, uint32_t addr, uint64_t val) {
    ...
    switch (addr) {
        ...
        // Supervisor CSRs
        case CSR_SSTATUS:       cpu->csr.sstatus = val; break;
        ...
        case CSR_SATP:          cpu->csr.satp = val; break;
        ...
        default:
            if (addr >= CSR_PMPCFG0  && addr <= CSR_PMPCFG0 + 0xF) {
